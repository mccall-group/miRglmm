...
library(foreach)
library(doParallel)
miRglmm_par <- function(se,  col_group = c(rep("A", 18), rep("B",19)), 
                    min_med_lcpm = -1){
  ...
  f1_results=foreach(ind3=1:10, .combine=rbind, .packages=c("tidyverse", "reshape2", "lme4", "SummarizedExperiment")) %dopar% {
    #cat(uniq_miRNA[ind3], "\n")
